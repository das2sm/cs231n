...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        # For W1, The filters make a 4D tensor of shape (F, C, HH, WW)
        # to change input from (C, H, W) -> (F, H, W) (assuming the stride and pad needed for this)
        # b1 is (F, )
        self.params['W1'] = np.random.normal(0, weight_scale, (num_filters, C, filter_size, filter_size))
        self.params['b1'] = np.zeros(num_filters)
        
        # W2 needs to be 2D but the input is 3D (F, H/2, W/2) from 2x2 max pooling
        # so we flatten it into 1D by multiplying
